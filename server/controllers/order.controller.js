...
exports.sellerConfirmOrder = asyncHandler(async (req, res) => {
    try {
        const order = await orderService.sellerConfirmOrder(req.user.userId, req.params.id);
        res.json({ message: 'Order confirmed. Escrow funds released to your balance.', order });
    } catch (error) {
        res.status(400).json({ error: error.message });
    }
});
