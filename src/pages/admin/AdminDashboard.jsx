...
import { BarChart3, Users, DollarSign, Activity, TrendingUp, ShoppingBag, Store, FileText, Shield } from 'lucide-react';
    ...
    useEffect(() => {
        async function load() {
            ...
            try {
                ...
                setError(e.message || t('admin.load_stats_error', 'Не удалось загрузить статистику'));
            } finally {
                setLoading(false);
            }
        }
        ...
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-center">
                    ...
                    <p className="text-slate-700">{t('common.loading', 'Загрузка...')}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 animate-in fade-in pb-10">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                {isAdmin ? (
                    <>
                        <Card
                            title="Общая Выручка"
                            value={`${(stats.totalRevenue || 0).toLocaleString()} Sum`}
                            icon={<DollarSign />}
                    ...
                    <>
                        <Card title={t('admin.stats.total_listings')} value={stats.listings} icon={<Activity />} sub={t('admin.active_ads', 'Активные объявления')} />
                        <Card title={t('admin.stats.total_sales')} value={`${(stats.revenue || 0).toLocaleString()} Sum`} icon={<DollarSign />} sub={t('admin.total_sales_volume', 'Общий объем продаж')} />
                        <Card title={t('admin.active_customers', 'Активные Клиенты')} value={stats.activeUsers} icon={<Users />} sub={t('admin.unique_buyers', 'Уникальные покупатели')} />
                        <Card title={t('admin.stats.total_orders')} value={stats.orders} icon={<BarChart3 />} sub={t('admin.successful_deals', 'Успешные сделки')} />
                    </>
                )}
            </div>
            ...
            <div className="grid gap-4 grid-cols-1 lg:grid-cols-7">
                {/* Revenue Chart */}
                <div className="col-span-1 lg:col-span-4 rounded-xl border bg-card shadow-sm p-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="rounded-xl border bg-card text-card-foreground shadow-sm">
                    <div className="p-6 pb-2">
                        <h3 className="text-lg font-semibold">{t('admin.top_products', 'Топ Товаров по Продажам')}</h3>
                    </div>
                    <div className="p-6 pt-2">
                        {(stats.topProducts?.length || 0) === 0 ? (
                            <p className="text-slate-700 text-sm">{t('admin.no_data', 'Данных пока нет')}</p>
                        ) : (
                            <div className="space-y-4">
            ...
            <div className="rounded-xl border bg-card text-card-foreground shadow-sm">
                <div className="p-6 flex flex-row items-center justify-between">
                    <h3 className="text-lg font-semibold">{t('admin.recent_orders', 'Недавние Заказы на Платформе')}</h3>
                </div>
                <div className="p-6 pt-0">
                    {(!stats.recentActivity || stats.recentActivity.length === 0) ? (
                        <p className="text-slate-700">{t('admin.no_orders', 'Заказов пока нет.')}</p>
                    ) : (
                        <div className="space-y-4">
